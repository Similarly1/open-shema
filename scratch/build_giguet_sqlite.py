...
gig_json_dir = r"c:\Users\adrie\Documents\antigravity\peaceful-mendeleev\bible_ai_app\data\bibles\GIG"
dest_dir = r"c:\Users\adrie\Documents\antigravity\peaceful-mendeleev\scratch\open-shema-data\data\bibles"
os.makedirs(dest_dir, exist_ok=True)
dest_db = os.path.join(dest_dir, "bible_septante_giguet.sqlite")
...
metadata_entries = [
    ("id", "bible-septante-giguet-1872"),
    ("type", "bible"),
    ("category", "bibles"),
    ("abbreviation", "GIG"),
    ("title", "La Sainte Bible d'après les Septante"),
    ("author", "Pierre Giguet / Lethielleux"),
    ("year", "1872"),
    ("language", "fr"),
    ("description", "La première et magistrale traduction française intégrale de l'Ancien Testament d'après la version des Septante (texte grec d'Alexandrie) réalisée par Pierre Giguet en 4 tomes (1872). Témoin capital du texte biblique cité par les auteurs du Nouveau Testament et les Pères de l'Église."),
...
json_files = sorted([f for f in os.listdir(gig_json_dir) if f.endswith('.json')])
print(f"Trouvé {len(json_files)} fichiers JSON pour Septante Giguet.")
...
for fname in json_files:
    fpath = os.path.join(gig_json_dir, fname)
...
print(f"Base SQLite Giguet générée avec succès : {dest_db} ({size_mb:.2f} Mo, {len(verses_data)} versets)")
conn.close()
