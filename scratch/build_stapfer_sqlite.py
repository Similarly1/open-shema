...
stapfer_json_dir = r"c:\Users\adrie\Documents\antigravity\peaceful-mendeleev\bible_ai_app\data\bibles\STAPFER"
dest_dir = r"c:\Users\adrie\Documents\antigravity\peaceful-mendeleev\scratch\open-shema-data\data\bibles"
os.makedirs(dest_dir, exist_ok=True)
dest_db = os.path.join(dest_dir, "bible_stapfer.sqlite")
...
metadata_entries = [
    ("id", "bible-stapfer-1889"),
    ("type", "bible"),
    ("category", "bibles"),
    ("abbreviation", "STAPFER"),
    ("title", "Le Nouveau Testament (Traduction Edmond Stapfer)"),
    ("author", "Edmond Stapfer (Librairie Fischbacher)"),
    ("year", "1889"),
    ("language", "fr"),
    ("description", "Traduction historique du Nouveau Testament par le théologien protestant Edmond Stapfer (1889, Librairie Fischbacher). Traduit directement sur le texte grec comparé des meilleures éditions critiques de l'époque (Tischendorf, Westcott-Hort, Tregelles), offrant une langue soignée et une grande rigueur philologique."),
...
json_files = sorted([f for f in os.listdir(stapfer_json_dir) if f.endswith('.json')])
print(f"Trouvé {len(json_files)} fichiers JSON pour Stapfer.")
...
for fname in json_files:
    fpath = os.path.join(stapfer_json_dir, fname)
...
print(f"Base SQLite Stapfer générée avec succès : {dest_db} ({size_mb:.2f} Mo, {len(verses_data)} versets)")
conn.close()
