...
// Let's find every regex replacement in articles_view.js
const regexMatches = [...articlesCode.matchAll(/text\s*=\s*([^;]+);/g)];
console.log(`Found ${regexMatches.length} text replacements`);
...
for (let i = 0; i < regexMatches.length; i++) {
  const expr = regexMatches[i][1];
  try {
    const fn = new Function('text', 'normalizeSuperscripts', 'bibleBooksPattern', 'notBibleRefAhead', 'solaImg', `return (${expr});`);
    const nextText = fn(text, normalizeSuperscripts, bibleBooksPattern, notBibleRefAhead, solaImg);
    if (!text.includes('2 Sam.\n') && !text.includes('2 Sam.</p>') && (nextText.includes('2 Sam.\n') || nextText.includes('2 Sam.</p>'))) {
      console.log(`\n===> CULPRIT FOUND AT INDEX ${i}:`);
      console.log(expr);
    }
    text = nextText;
  } catch (e) {
    // console.log("Skipping expression:", e.message);
  }
}
